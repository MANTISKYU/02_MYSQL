...
select
    bit_length('pie'), char_length('pie'), length('pie');

select
    tbl_menu.menu_name,bit_length(tbl_menu.menu_name), char_length(tbl_menu.menu_name), length(tbl_menu.menu_name)
from
    tbl_menu;
...
-- format(숫자, 소숫점 자릿수)
-- format : 1000 단위마다 콤마(,) 표시해주며, 소숫점 아래 자릿수(반올림)까지 표현한다.
...
-- RTRIM(문자열) : 오른쪽 공백 제거
-- TRIM(문자열) : 양쪽 공백을 제거
-- TRIM(BOTH, LEADING(왼쪽), TRAILING(오른쪽)) 있으면 해당 방향에 지정한 문자열을 제거 할 수 있다.
select ltrim('     왼쪽'),
       rtrim('오른쪽     '),
       trim('       양쪽      '),
...
-- substring(문자열, 구분자, 횟수) : 구분자가 왼쪽부터 횟수 번째 나오면 그 이후의 오른쪽은 버린다.
...
select
    adddate('2024-06-13', interval 30 day),
...
select
    addtime('2024-06-13 12:00:00', '1:0:1'),
    subtime('2024-06-13 12:00:00', '1:0:1');

-- 현재 날짜 연-월-일 시간-분-초 반환해주는 함수들
