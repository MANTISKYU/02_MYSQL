...
-- 서브쿼리의 규칙
-- 서브쿼리는 반드시 소괄호()로 묶여야한다. (SELECT ...) 형태
...
where
    ...
    select
        tbl_menu.menu_code 메뉴코드,
        tbl_menu.menu_name 메뉴명,
        tbl_menu.menu_price 메뉴가격,
        tbl_menu.orderable_status 주문가능여부
    from
        tbl_menu
    where
        category_code = 4;
...
-- 메뉴명과 카테고리명을 함께 출력
with menucate as (select menu_name, category_name
                  from tbl_menu a
                           join
